...
def _reinforcement_points(Nx, Ny, x, y, addx, z, margin):
    ...
    reinf_tags_2 = (
        []
    )  # create a second list for the case "both" (makes creating lines later easier)
    ...
    x_cords_2 = np.linspace(
        x[0], x[-1], len(x)
    )  # add additional x coordinates for lines in x direction
    y_cords_2 = np.linspace(
        y[0], y[-1], Ny
    )  # additional y coordinates for lines in x direction
...
def _reinforcement_lines(reinf_tags, reinf_tags_2, Nx, Ny, elems_x, elems_y):
    ...
    def _create_lines_from_tags(
        reinf_tags, line_tags, ltl, k_tags, k_x_or_y, counter_y, counter_x
    ):
        for i, point_tag in enumerate(reinf_tags):
            ...
            if (
                k_x_or_y < len(reinf_tags) - (elems_x + 1) * Ny
                ...
                if counter_y == elems_y + 1:
                    ...
                    gmsh.model.occ.addLine(
                        point_tag, reinf_tags[i + 1], tag=ltl + k_tags
                    )
                    line_tags.append(ltl + k_tags)
            elif (
                k_x_or_y < len(reinf_tags)
                and k_x_or_y > len(reinf_tags) - (elems_x + 1) * Ny
    ...
    ltl, k_tags, line_tags = _create_lines_from_tags(
        reinf_tags, line_tags, ltl, k_tags, k_x_or_y, counter_y, counter_x
    )

    if len(reinf_tags_2) > 0:

        _, _, line_tags = _create_lines_from_tags(
            reinf_tags_2, line_tags, ltl, k_tags, k_x_or_y, counter_y, counter_x
        )
    return line_tags
